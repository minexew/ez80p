;----------------------------------------------------------------------------------------
; Convert Amiga hardware values from eZ80 Protracker Player to AMOEBA spec
; and writes to hardware registers
;-----------------------------------------------------------------------------------------
;
; V1.02 - Changed audio register writes to AMOEBA v10A spec 
; V1.01 - ADL mode
;
; Writes 4 tracker channels to left channels
;----------------------------------------------------------------------------------------
...
update_audio_hardware
				...
				ld a,0ffh							
				ld (hw_audio_registers+80h),a		;lock all audio registers
				ld (a_chan_restarted),a

				ld ix,channel_data
				ld iy,hw_audio_registers
				ld b,4
				
reg_loop1		bit 0,(ix+control_bits)				; was a new sample triggered on this channel?
				jr z,no_trig
				ld de,(ix+sample_location)			; update location register
				ld (iy),de
				ld de,(ix+sample_length)			; update length register
				ld (iy+4),de
				ld (iy+0eh),0						; clear the swap flag
				ld (iy+0fh),011b					; swap on loop / restart channel
				ld a,4
				sub a,b
				ld (a_chan_restarted),a				; make a note of the last channel that restarted
				
no_trig			ld de,(ix+period_lo)				; convert Amiga period to AMOEBA freq constant
				ld hl,0
				ld l,e
				ld h,d
				add hl,hl
				ld de,period_conv_table-200		
				add hl,de
				ld de,0
				ld e,(hl)
				inc hl
				ld d,(hl)
				ld (iy+08h),de						; update period register
				
				ld a,(ix+volume)				
				ld (iy+0ch),a						; update volume register 
				
				lea ix,ix+vars_per_channel
				lea iy,iy+16
				djnz reg_loop1
				xor a
				ld (hw_audio_registers+80h),a		; unlock all audio registers

;--------------------------------------------------------------------------------------------------------------

				ld a,(a_chan_restarted)				;if no channel restarted we dont need to do the second part
				cp 0ffh
				ret z
				out0 (port_selector),a				;set the address of the channel swap register we want to examine
wait_audio		in0 a,(port_hw_flags)
				bit 6,a
				jr z,wait_audio
...
;--------------------------------------------------------------------------------------------------------------
				...
				ld iy,hw_audio_registers
				ld b,4
reg_loop2		bit 0,(ix+control_bits)
				jr z,no_trig2
				res 0,(ix+control_bits)
				
				ld de,(ix+sample_loop_location)		; set loop location register
				ld (iy),de
				ld de,(ix+sample_loop_length)		; set loop length register
				ld (iy+4),de
				ld (iy+0eh),0
				ld (iy+0fh),000b					; dont swap on loop, no restart

no_trig2		lea ix,ix+vars_per_channel
				lea iy,iy+16
				djnz reg_loop2
				
				xor a
				ld (hw_audio_registers+80h),a		;unlock audio registers
