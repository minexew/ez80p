...
;---------------------------------------------------------------------------------------------
				...
				call init_mouse					; ZF set if all OK, else A = error code
				jr z,mok
...
mouse_sample_rate		equ 100			; 100 samples per second, valid: 10,20,40,60,80,100,200
mouse_resolution		equ 3			; 8 counts per mm, valid: 00h-03h
mouse_scaling			equ 0e6h		; valid commands 0e6h (1:1) / 0e7h (2:1)

intellimouse_seq	db 0f3h,200
					db 0f3h,100
					db 0f3h,80

mouse_settings_seq	db 0e8h,mouse_resolution
					db mouse_scaling
					db 0f3h,mouse_sample_rate
					db 0f4h
					
;-----------------------------------------------------------------------------------------------

text_here			db "Testing..",11,0
timeout_txt			db "Mouse error!",11,0
