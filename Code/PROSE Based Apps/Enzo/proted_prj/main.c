...
#define VERSION			"1.2"					/* Version number of this editor.                      */
#define FUDGE			10
#define FALSE			0
#define TRUE			1
...
void main(void)
{
	...
	if (get_prose_version() < 0x31)
	{
		print("Proted require PROSE ver 31 or later!\n\r");
...
/* Setup PROSE screen dimension at 80x60 characters */
void set_display_size(void)
{
	asm ("push ix");
	asm ("ld e, 0h");
	asm ("ld a, kr_set_video_mode");
	asm ("call.lil prose_kernal");
	asm ("pop ix");
}
...
void msg_window(char *msg1, char *msg2)
{
	...
	memset(&backbuffer[0][0], 0, sizeof(backbuffer));
